...
def resnext_50(x):
    def stack_fn(x):
        x = stack(128, 3, stride1=1, name='conv2')(x)
        x = stack(256, 4, name='conv3')(x)
        x = stack(512, 6, name='conv4')(x)
        x = stack(1024, 3, name='conv5')(x)
        return x
...
def block(filters, kernel_size=3, stride=1, groups=32,
...
def stack(filters, blocks, stride1=2, groups=32, name=None):
    ...
    def build(x):
        with K.name_scope(name):
            x = block(filters, stride=stride1,
                      groups=groups, name=name + '_block1')(x)
